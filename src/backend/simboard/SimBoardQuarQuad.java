...
public class SimBoardQuarQuad extends SimBoardQuad implements Quarantinable {
    ...
    private int width, height, xShift, yShift, x2Start, y2Origin, quarXOrigin, quarWidth;
    ...
    public void drawSepLines(Graphics2D g2D) {
        ...
        int segmentXLen = (getDimens().width - quarantine.width - xShift/5)/31;
        g2D.setColor(CustomColor.EERIE_BLACK);

        for(int i = 0; i < 25; i++)
        {
            if(i%2 == 0)
                g2D.fillRect(x2Start - xShift/12, getDimens().y + i*segmentYLen, segmentWidth, segmentYLen );
        }
        for(int i = 0; i < 31; i++)
        {
            if(i%2 == 0)
